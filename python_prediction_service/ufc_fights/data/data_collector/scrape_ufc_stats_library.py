...
library of functions to scrape ufc_fights stats
...
def parse_upcoming_fight_details(soup):
    '''
    ...
    for tag in soup.find_all('a', class_='b-link b-link_style_black'):
        ...
        if name != 'View Matchup':
            fighters_in_event.append(name)

    # Filter out matchups with "View Matchup"
    fighters_in_event = filter_view_matchup(fighters_in_event)
    ...
    fight_details_df = pd.DataFrame({'BOUT': fighters_in_event, 'URL': fight_urls})
...
def filter_view_matchup(fighters_in_event):
    filtered_fighters = []
    for fighter_a, fighter_b in zip(fighters_in_event[::2], fighters_in_event[1::2]):
        if fighter_a != "View Matchup" and fighter_b != "View Matchup":
            filtered_fighters.append(fighter_a + ' vs. ' + fighter_b)
