...
def get_ufc_link(query):
    ...
    for url in possible_urls:
        if ("ufc_fights.com/athlete/" in url):
            return url
...
def get_past_event_links():
    ...
    xml = html.document_fromstring(htm.content)
    return ["https://www.ufc.com/" + x for x in xml.xpath(
        "//details[@id='events-list-past']/div/div/div/div/div/section/ul/li/article/div[1]/div/a/@href")]


def get_ufc_link_event(query):
    possible_urls = search(query + " UFC")
    for url in possible_urls:
        if ("ufc_fights.com/event/" in url):
            return url
    raise BaseException("UFC link not found !")
...
def parse_event(url, past=True):
    ...
    if past:
        ...
        fights_html = xml.xpath("//div[@class='l-main']/div/div/div/div/section/ul/li")
    ...
    for fight in fights_html:
        fight_text = ET.tostring(fight, encoding='unicode', method='text').strip()
        #  print(fight_text)
        this_fight = {
...
def get_past_events():
    ...
    links = get_past_event_links()

    for url in links:
        try:
            event = parse_event(url, True)
            print(event)
            results[event['name']] = event
...
def get_event(query, past=True):
    link = get_ufc_link_event(query)
    return parse_event(link, past)
